...
class Plang(db.Model): 
    ...
    plang_description = db.Column(db.String(255), nullable=True)
...
class UserLang(db.Model): 
    ...
    langdeets = db.relationship("Plang", backref="langusers")
...
class Purchases(db.Model):
   ...
   purchase_user_id = db.Column(db.Integer(),db.ForeignKey("user.user_id"))
   product_id = db.Column(db.Integer(),db.ForeignKey("products.product_id"))
   pur_trxid = db.Column(db.Integer(),db.ForeignKey("transaction.trx_id"))

   productdeets = db.relationship("Products", backref="users")
...
class Posts(db.Model):
   ...
   post_user_id = db.Column(db.Integer(),db.ForeignKey("user.user_id"))
...
class Comments(db.Model):
   ...
   comment_postid = db.Column(db.Integer(),db.ForeignKey("posts.post_id"))
   comment_by = db.Column(db.Integer(),db.ForeignKey("user.user_id"))
   comment_date = db.Column(db.DateTime(), default=datetime.datetime.utcnow())

   commentuser = db.relationship("User", backref="comments")
   postcommented = db.relationship("Posts", backref="mycomments")
